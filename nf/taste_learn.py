...
입력으로 (a) 현재 taste_profile.md 전문, (b) PD 선택/폐기/수정/직접편집 신호 다이제스트를 받습니다.
...
- 'PD 직접 편집'은 가장 강한 신호입니다. 자주 뺀 표현은 회피 패턴·문체 선호로, 자주 넣은 요소는 살려야 할 재미 요소·문체 선호로 반영합니다.
...
def summarize_signals(signals: list[dict]) -> str:
    ...
    for signal in signals:
        ...
        if action == "pd_edit":
            for removed in _as_list(signal.get("removed")):
                text = _clean_text(removed)
                if text:
                    pd_removed.append(text)
            for added in _as_list(signal.get("added")):
                text = _clean_text(added)
                if text:
                    pd_added.append(text)
    ...
    lines.extend(["", "## PD 직접 편집 — 퇴고에서 뺀 것"])
    if pd_removed:
        for text in pd_removed[:30]:
            lines.append(f"- (뺌) {text}")
    else:
        lines.append("- 없음")

    lines.extend(["", "## PD 직접 편집 — 퇴고에서 넣은 것"])
    if pd_added:
        for text in pd_added[:30]:
            lines.append(f"- (넣음) {text}")
    else:
        lines.append("- 없음")
