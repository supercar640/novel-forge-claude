...
**Novel Forge Claude (NFC) v1.7** — PD(기획자)와 AI가 협업하여 웹소설을 기획하고 집필하는 인터랙티브 작성 도구. 코드 프로젝트가 아닌 **AI 주도의 창작 워크플로우**이며, 전체 스펙은 `NFC_plan.md`에 정의되어 있다.
...
- **Phase 3**: 문체 설정 → 작성 모드 선택 (자동작성/장면별/1화 분량, 상호 배타) → 집필 → **언제든 auto 전환 가능**
...
- `H <id>`: `python nfc.py hold <id>` → shelve/idea_*.md로 자동 저장
...
- `H <id>`: `python nfc.py hold <id>` → shelve/dev_*.md로 자동 저장
- `D <id>`: `python nfc.py discard <id>`
...
**AI가 할 일:**
1. 사용자에게 작성 모드를 선택하도록 요청한다. 3가지 모드를 안내:
   ...
   auto와 scene/episode는 상호 배타적 — 하나만 설정 가능
...
6. 사용자에게 원고를 제시하고 의사결정을 요청한다:
   ```
   [A]pprove(승인) / [M]odify(수정) / [D]ismiss(폐기) / [H]old(보류)
...
- `H`: `python nfc.py hold` → 현재 초안을 `shelve/draft_*.md`로 저장, 집필 단계로 복귀하여 재작성 (v1.7)
...
- `H`: `python nfc.py hold` → 현재 초안을 shelve/draft_*.md로 저장, 재작성
...
PD가 이미 완성된 에피소드를 수정하고 싶을 때 사용한다.

**사용 가능 단계**: `development_proposal` (Phase 2 시작), `complete` (회차 완료)

**AI가 할 일:**
1. 사용자가 수정할 에피소드를 지정한다:
   ```bash
   python nfc.py revise-episode ep001.md
   ```
2. 에피소드가 `drafts/revision_ep001.md`로 복사되고 `writing_decision` 단계로 이동한다
3. 사용자에게 원고를 제시하고 의사결정을 요청한다:
   ```
   [A]pprove(승인) / [M]odify(수정) / [D]ismiss(폐기) / [H]old(보류)
   ```
4. 수정 완료 후 Phase 4(퇴고 → 컨텍스트 갱신)를 거쳐 원본 에피소드를 덮어쓴다
5. **episode_count는 변하지 않는다** (재수정이므로)
6. 완료 후 원래 단계로 복귀한다
...
| `revise-episode <file>` | — | **v1.7** 완성된 에피소드를 수정 모드로 불러오기 |
...
- **v1.7 shelve 관리**: hold된 아이디어(idea_*), 전개(dev_*), 초안(draft_*)은 shelve/ 디렉토리에 자동 저장
- **v1.7 과거 회차 재수정**: `revise-episode`로 완성 에피소드 수정 가능, episode_count 변동 없음
